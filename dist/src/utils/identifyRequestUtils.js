...
export const formatPlantNetRequest = (images, apiUrl, apiKey) => {
...
export const formatC4CRequest = (images, apiUrl, apiKey) => {
    const form = new FormData();
...
export const formatCarpesoRequest = (images, apiUrl, apiKey) => {
    const form = new FormData();
    for (let i = 0; i < images.length; i += 1) {
        form.append('image', images[i]);
    }
...
export const formatPlantNetResponse = async (response) => {
    const responseJson = await response.json();
    if ('error' in responseJson) {
        return responseJson.message;
    }
    return {
        results: responseJson.results.map((result) => ({
            score: result.score,
...
export const formatC4CResponse = async (response) => {
    const responseJson = await response.json();
    ...
    const responseJsonResults = responseJson.results;
    return {
        results: responseJsonResults.map((result) => ({
            score: result.score,
...
export const formatCarpersoResponse = async (response, apiUrl) => {
    const responseJson = await response.json();
    if ('status' in responseJson && responseJson.status === 'error') {
        return responseJson.status;
    }
...
//# sourceMappingURL=identifyRequestUtils.js.map